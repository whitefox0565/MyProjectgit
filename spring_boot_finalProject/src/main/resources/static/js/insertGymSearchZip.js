 ...
 */
 ...
 $(document).ready(function(){
    ...
    var gymNameChk = false; // 도장이름 확인
    var priceChk = false; // 월회비 확인
    //var gymHpChk = false; // 도장 전화번호 확인
   //var memHpChk = false; // 회원 전화번호 확인
   ...
   $('#memEmail').on('keyup', function(e){
         ...
         if(emailPattern.test(email) && e.keyCode >= 0) {
            ...
            emailChk = false;
            $('#memEmail_chk').empty();
            $('#memEmail_chk').text('올바른 이메일이 아닙니다');   
         }
   });
   
   //도장이름
   $('#gymName').on('keyup',function(e){
      let gymName = $(this).val();
      let namePattern = /^[a-zA-Z0-9가-힣\s]{3,20}$/;
      if(gymName.length <= 0) $('#gymName_chk').empty();
      
      if(namePattern.test(gymName) && e.keyCode >= 0){
         gymNameChk = true;
         $('#gymName_chk').empty();
         $('#gymName_chk').text ('올바른 형식입니다').css('color', '#03C75A');
         if(e.keyCode == 13) $('#gymHp').focus();
      }else{
         gymNameChk = true;
         if(gymName.length > 0){
            $('#gymName_chk').empty();
            $('#gymName_chk').text('올바른 형식이 아닙니다').css('color', 'red');
         }
      }
   });
   
   //도장 번호
   $('#gymHp').on('keyup', function(e){
         let hp1 = $('#gymHp').val();
         let hp1Pattern = /^(0[0-9]{10})$/;
         if(hp1Pattern.test(hp1) && e.keyCode == 13) {
            $('#gymPrice').focus();               
         }
   });
   ...
   //월회비
   $('#gymPrice').on('keyup', function(e){
      let price = $('#gymPrice').val();
      let pricePattern = /^1[0-9]{4,}$/;
      if(pricePattern.test(price)) priceChk = true;
   });
